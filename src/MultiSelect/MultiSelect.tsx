...
const MultiSelect = (props: any) => {
  ...
  const filterOptions = (options: Option[], input: string) =>
    options?.reduce((acc: Option[], option: Option) => {
      if (option.group) {
        const filteredGroupOptions = option.group.filter(({ label }) =>
          label.toLowerCase().includes(input.toLowerCase())
        );
        if (filteredGroupOptions.length > 0) {
          acc.push({ ...option, group: filteredGroupOptions });
        }
  ...
  const handleChange = (selected: Option[]) => {
    // Adicionar o próprio objeto com grupo à seleção e todos os objetos do grupo que não estão selecionados
    if (selected.length > 0) {
      const lastSelectedOption = selected[selected.length - 1];
      if (lastSelectedOption && lastSelectedOption.group) {
        const groupOptions = lastSelectedOption.group;
        const updatedSelection = [
          ...
          ...groupOptions.filter(
            (groupOption) =>
              !selected.some((sel) => sel.value === groupOption.value)
          ),
        ];
    ...
    // Remover apenas o objeto desmarcado do grupo da seleção
    const lastDeselectedOption = props.value?.find(
      (option: any) => !selected.some((sel) => sel.value === option.value)
    );
    if (lastDeselectedOption && lastDeselectedOption.group) {
      const groupValues = lastDeselectedOption.group.map(
        (opt: Option) => opt.value
      );
      const updatedSelection = selected.filter(
        (option) =>
          option.value !== lastDeselectedOption.value &&
          !groupValues.includes(option.value)
      );
      selected = updatedSelection;
    }

    // Atualizar o estado com a seleção final
    props.onChange(selected);

    // Lógica adicional de atualização conforme necessidade
    if (
      selected.length > 0 &&
      !isAllSelected.current &&
  ...
  if (props.isSelectAll && props.options.length !== 0) {
    ...
    if (filteredSelectedOptions?.length > 0) {
      if (filteredSelectedOptions?.length === filteredOptions?.length)
        ...
        selectAllLabel.current = `${filteredSelectedOptions?.length} / ${filteredOptions.length} SELECIONADO(S)`;
